        ...
        if (user) {
            if (name) {
                ...
                if (existingName && String(existingName._id) !== String(user._id)) {
                    res.status(400).json({
                        success: false,
                        message: types_1.EThisNameIsNotAvailable[req.body.language] ||
...
const addToBlacklistedJokes = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        let user;
        if (value) {
            user = yield user_1.User.findOneAndUpdate({ _id: id }, { $push: { blacklistedJokes: { jokeId, language, value } } }, { new: true });
        }
        else {
            user = yield user_1.User.findOneAndUpdate({ _id: id }, { $push: { blacklistedJokes: { jokeId, language } } }, { new: true });
        }
        if (user) {
            res.status(200).json({
                success: true,
...
exports.removeJokeFromBlacklisted = removeJokeFromBlacklisted;
